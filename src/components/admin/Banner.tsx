...
import { Plus, Trash2, Edit3, X, Image as ImageIcon, Loader2, CheckCircle, Clock } from 'lucide-react';
...
export const BannerManagement: React.FC = () => {
  ...
  const fetchBanners = async (page = 1) => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/banners?page=${page}&limit=8`);
      if (response.ok) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { toast.warning('VUI LÒNG NHẬP TÊN CHIẾN DỊCH'); return; }
    if (!image) { toast.warning('VUI LÒNG CHỌN HÌNH ẢNH BANNER'); return; }
  ...
  const handleDelete = async (id: number) => {
    const confirmed = await confirmModal('BẠN CÓ CHẮC CHẮN MUỐN XÓA BANNER NÀY KHÔNG?');
    if (!confirmed) return;
    try {
  ...
  const getStatusBadge = (statusVal: number) => {
    if (statusVal === 1) {
      return (
        <span className="inline-flex items-center gap-1 border border-emerald-950 bg-emerald-950/20 px-2.5 py-0.5 text-[8px] font-mono font-bold tracking-widest text-emerald-400 uppercase">
          <CheckCircle className="h-2.5 w-2.5" /> KÍCH HOẠT
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 border border-zinc-850 bg-zinc-900 px-2.5 py-0.5 text-[8px] font-mono font-bold tracking-widest text-zinc-500 uppercase">
        <Clock className="h-2.5 w-2.5" /> TẠM ẨN
      </span>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-zinc-900 pb-6">
        <div>
          <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-zinc-550">MARKETING SERVICES</span>
            ...
            <table className="w-full text-left font-mono text-xs text-zinc-400 border-collapse">
              <thead>
                <tr className="border-b border-zinc-900 text-[10px] text-zinc-550 uppercase tracking-widest">
                  <th className="pb-3 font-semibold">MÃ BANNER</th>
                  <th className="pb-3 font-semibold">HÌNH ẢNH MINH HỌA</th>
                  <th className="pb-3 font-semibold">TÊN CHIẾN DỊCH / BANNER</th>
              ...
              <tbody className="divide-y divide-zinc-950">
                {banners.map((banner) => (
                  <tr key={banner.id} className="hover:bg-zinc-900/20 transition-colors">
                    {/* ID */}
                    <td className="py-3.5 text-zinc-500">#{banner.id}</td>

                    {/* Image */}
                    <td className="py-3.5">
                      <div className="h-12 w-28 border border-zinc-900 bg-zinc-950 flex items-center justify-center p-1 overflow-hidden">
                        {banner.image ? (
                          <img
                            src={banner.image}
                            alt={banner.name}
          ...
          <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="font-display text-base font-black uppercase tracking-tight mb-4 border-b border-zinc-900 pb-2">
                {editingBanner ? 'CẬP NHẬT BANNER' : 'THÊM CHIẾN DỊCH BANNER'}
              </h3>

              <form onSubmit={handleSave} className="space-y-4 font-mono text-xs">
                {/* Banner Name */}
                <div className="space-y-1">
                  <label className="block text-[9px] text-zinc-555 uppercase tracking-widest">Tên Banner / Chiến dịch</label>
                  <input
                    type="text"
                    required
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="E.G. NEW ARRIVALS SPRING 2026..."
                ...
                {/* Status Toggle */}
                <div className="space-y-1">
                  <label className="block text-[9px] text-zinc-555 uppercase tracking-widest">Trạng thái chiến dịch</label>
                  <select
                    value={status}
                ...
                <div className="space-y-2">
                  <label className="block text-[9px] text-zinc-555 uppercase tracking-widest">Hình ảnh banner</label>
                  
                  {image ? (
                    <div className="relative border border-zinc-850 p-2 bg-zinc-900/30 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-24 border border-zinc-900 bg-black p-1">
                          <img src={image} alt="Selected Banner" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                        </div>
